...
surveys~\cite{wang2024survey,guo2024llmmultiagent,tran2025multiagent,hossain2026safe}
document a common architecture of roles, messages, and tools, and identify
layered trust as a recurring organizing principle. These works focus on
\emph{capability}, whereas we focus on \emph{control}: bounding what a
collective is permitted to do as a function of measured trust. Our framework
is complementary, protocol-agnostic, and can wrap any of these systems.
...
scope, and revocability---and shows that increasing \emph{scalar} trust monotonically
raises both over-exposure and authorization drift. Xu et~al.'s defences accept scalar
trust as given and reduce exposure by tuning \emph{around} it (thresholding,
rate-limiting) rather than reshaping how the signal is computed. TGCC differs on three
axes: (i)~we lift trust from a scalar to a coupled layered vector, so an epistemically
broken but behaviourally polished agent is legible at all; (ii)~we prove the paradox is
broken (Theorem~\ref{thm:paradox}) at the operator level, not merely observed
empirically; and (iii)~we ship a runtime capability gate with logarithmic revocation
latency (Corollary~\ref{cor:revoke}) and exponentially decaying honest false positives
(Theorem~\ref{thm:fp}). Zero-trust treatments for agentic
AI~\cite{zerotrust2025,nist800207} and TRiSM~\cite{trism2026} advocate verifiable
identity and continuous verification but stop short of a concrete multi-layer decision
rule; we supply exactly the missing rule.
...
prerequisite failure visible. Input/output content-filter systems such as
LLM-Guard~\cite{llmguard2024} and NeMo Guardrails~\cite{nemoguardrails2023} operate on a
different axis (per-message text screening) and are complementary to any runtime trust
controller; TGCC composes with them rather than replacing them.
...
calibration is cheap in practice. Calibration fixes only the numerical thresholds; the
paradox-resolution, latency, and false-positive guarantees
(Thms.~\ref{thm:paradox},~\ref{thm:fp}; Cor.~\ref{cor:revoke}) hold irrespective of the
specific threshold values chosen.
\end{remark}
...
accuracy keeps the epistemic signal high but pays a persuasion cost: E16
(Appendix~\ref{app:w16}) verifies that perfect calibration raises OER only from $0.17$ to
$\approx\!0.22$ while attacker confidence collapses to $\rho\!\approx\!0.30$, well below
the persuasion threshold of any high-risk capability, so the threat reduces to
low-quality output the role and social layers are designed to catch.
\end{remark}
...
Appendix~\ref{app:figures}). Real-LLM experiments use two production LLMs (Anthropic
Claude Sonnet~4.5 and Azure OpenAI \texttt{gpt-5}) through their APIs with per-layer
checks derived from LLM probes; cross-provider bootstrap CIs are reported in
Appendix~\ref{app:w6}. Simulations draw Bernoulli check signals from latent
reliabilities. We compare three conditions throughout: the full
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/exp1_results}}
...
collapses, and the controller detects and revokes in every case.

\begin{table}[t]
\caption{Consolidated results. Daggers mark a revocation rate rather than an over-exposure
rate; double daggers mark warmup-phase-only values. \textbf{Bootstrap 95\% CIs}
(Appendix~\ref{app:w6}) for OER on the E1 signal streams: \textsc{TGCC}~$[0.08,0.25]$;
scalar-trust baselines $\ge[0.53,1.00]$.}
...
axes not covered by E1--E5.  A stratified bootstrap on the E1 streams
(E6) places TGCC's OER $95\%$ CI strictly below every scalar-trust
baseline on both providers ($p<0.01$ vs.\ four of five competitors).
A three-persona probe (E7) exposes a \emph{negative result}: a naive
cross-agent agreement signal backfires and denies the honest agent.
Both LLMs resist prompt injection (E8), and per-layer ablation (E9)
shows spoofing the epistemic signal restores OER to $1.00$ while
spoofing any other layer leaves it at $0.17$--$0.25$.  E10--E16
cover drift, sensitivity, recovery, Byzantine chains, audit trails,
calibration, and adaptive attackers, each with a formal statement
(Appendix~\ref{app:robustness_ext}).

% ══════════════════════════════════════════════════════════════════════
\section{Deployment and Protocol Integration}
\label{sec:discussion}
% ══════════════════════════════════════════════════════════════════════
Interoperability protocols~\cite{mcp2024,a2a2025,interop2025} expose
agent capabilities as token-mediated grants but leave the issuance
policy unspecified; TGCC's grant rule (Definition~\ref{def:grant})
supplies it (Appendix~\ref{app:casestudy}: worked clinical
deployment).
...
\label{sec:disc_open}
% ══════════════════════════════════════════════════════════════════════
We have turned a descriptive layered trust model into a deployable, provably
sound controller: TGCC breaks the Trust-Vulnerability Paradox with
logarithmic revocation latency, exponentially rare honest-agent false
positives, and per-agent cost constant in collective size. Open directions
include learning the coupling matrix from traces, multi-hop trust chains,
and a game-theoretic attacker--defender analysis; colluding adversaries and
controller compromise are out of scope
(Appendices~\ref{app:taxonomy},~\ref{app:robustness_ext}).

\section*{Ethics and Privacy Statement}
This \emph{defensive} paper uses only synthetic tasks and evaluates a mitigation against
the stealth-sleeper threat model~\cite{hubinger2024sleeper,qi2025trust}. An anonymized
reviewer repository (source, E1--E16 harnesses, per-turn logs) is at
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/trust_traj}}
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/exp2_debate_amplification}}
...
\begin{figure*}[h]
\centering\includegraphics[width=0.92\textwidth]{figures/eval}
...
\begin{figure*}[h]
\centering\includegraphics[width=0.92\textwidth]{figures/exp3_ablation_heatmaps}
...
\begin{figure*}[h]
\centering\includegraphics[width=0.92\textwidth]{figures/exp4_pipeline}
...
\begin{figure*}[h]
\centering\includegraphics[width=0.92\textwidth]{figures/exp5_scalability}
...
\begin{figure*}[t]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w6_bootstrap_ci}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w7_collusion}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w8_prompt_injection}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w9_ablation}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w10_long_horizon}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w11_sensitivity}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w12_recovery}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.92\textwidth]{figures/w13_byzantine_chain}}
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/w15_calibration_rms}}
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/w16_adaptive_attacker}}
